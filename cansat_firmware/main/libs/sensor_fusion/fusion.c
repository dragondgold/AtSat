                         ...
                         struct GyroSensor *pthisGyro)
{
    ...
    ESP_LOGV(TAG, "Kalman 6DOF GY start");

    // if requested, do a reset initialization with no further processing
    if (pthisSV->resetflag)
    {
        ESP_LOGV(TAG, "Kalman 6DOF GY reset");
    ...
    {
        ESP_LOGV(TAG, "Kalman 6DOF GY FIFO not empty");
    ...
    {
        ESP_LOGV(TAG, "Kalman 6DOF GY FIFO empty");
    ...
    ESP_LOGV(TAG, "sqrt: %.4f %.4f %.4f %.4f", pthisAccel->fGc[CHX],  pthisAccel->fGc[CHY], pthisAccel->fGc[CHZ], fmodGc);
    if (fmodGc != 0.0F)
    {
        ESP_LOGV(TAG, "Kalman 6DOF GY no free-fall");
    ...
    {
        ESP_LOGV(TAG, "Kalman 6DOF GY free-fall");
...
#if THISCOORDSYSTEM == NED
    ESP_LOGV(TAG, "Kalman 6DOF GY calculating NED angles.");
