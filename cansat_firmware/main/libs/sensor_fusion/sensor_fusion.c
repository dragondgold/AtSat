...
void queueStatus(SensorFusionGlobals *sfg, fusion_status_t status)
{
    ...
    ESP_LOGV(TAG, "Queue status: %d", status);
}
...
void initializeFusionEngine(SensorFusionGlobals *sfg)
{
    ...
    */
    ...
    ESP_LOGV(TAG, "Initializing fusion");
...
#if F_USING_ACCEL
    fInitializeAccelCalibration(&sfg->AccelCal, &sfg->AccelBuffer, NULL );
#endif
