...
const generateLineal = () => {
  return 10
}

const generateYLabels = () => {
  const flip = !!Math.floor(Math.random() * 2)
  return flip ? ['Debit', 'Credit', 'threshold'] : ['threshold','Credit', 'Debit']
}
...
const generateArrayLineal = (length) => {
  return Array.from(Array(length), generateLineal)
}
...
export const getLineChartData = () => {
  ...
  return {
    ...
    datasets: [
      ...
      {
        label: yLabels[2],
        backgroundColor: utils.hex2rgb('#ff0000', 0.6).css,
        borderColor: palette.transparent,
        data: generateArrayLineal(size),
      },
    ],
  }
}
