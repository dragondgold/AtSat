    ...
    <div class="form-elements chart-widget">
        ...
        <div class="col-md-12">
            <vue-chart style="min-height:15rem"
                ...
                <a v-if="debug" href @click.prevent="addBtn">
                        Agregar muestra
                </a>  
        </div>
    </div>
    ...
    methods:{
        ...
        loadData(){
                ...
                for(let s = 0; s < this.sensor.samples.length; s++){
                    if( this.sensor._type != "vector"){
                        this.addNewData(this.sensor.samples[s].lastValue,this.sensor.minValue,this.sensor.maxValue,this.sensor.samples[s].timespan)
                    }else{
                        this.addNewData(this.sensor.samples[s].x,this.sensor.samples[s].y,this.sensor.samples[s].z,this.sensor.samples[s].timespan)
                    }
                }
        ...
        addNewData(x,y,z,time){
            if(x != undefined){ // 'x' or 'lastValue' depend of sensors _type
                ...
                if(this.dataOrig.x.length > this.limit && !this.showAllSamples){
                    this.data.datasets[0].data = this.dataOrig.x.slice(Math.max(this.dataOrig.x.length - this.limit, 0))
                }else{       
            ...
            if(y != undefined){
                ...
                if(this.dataOrig.y.length > this.limit && !this.showAllSamples){
                    this.data.datasets[1].data = this.dataOrig.y.slice(Math.max(this.dataOrig.y.length - this.limit, 0))
                }else{       
            ...
            if(z != undefined){
                ...
                if(this.dataOrig.z.length > this.limit && !this.showAllSamples){
                    this.data.datasets[2].data = this.dataOrig.z.slice(Math.max(this.dataOrig.z.length - this.limit, 0))
                }else{       
            ...
            if(time != undefined){
                ...
                if(this.dataOrig.time.length > this.limit && !this.showAllSamples){
                    this.data.labels = this.dataOrig.time.slice(Math.max( this.dataOrig.time.length - this.limit, 0))
                }else{       
