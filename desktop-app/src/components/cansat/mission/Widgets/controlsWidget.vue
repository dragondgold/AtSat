          ...
          <div class="col-md-12">
            <table class="table table-striped first-td-padding col-md-12">
              <thead>
                <tr>
                  <td
                    v-if="!isImportedMission"
                  >{{$t('cansat.mission.dashboard.controls.projectName')}}</td>
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <vuestic-tooltip
                      v-if="!isImportedMission && !isMissionActive && !finished"
                      :options="{ 'content': $t('cansat.mission.dashboard.controls.startBtn'), 'placement': 'bottom'  }"
                    >
                      <div
                    ...
                    <vuestic-tooltip
                      v-if="!isImportedMission && isMissionActive"
                      :options="{ 'content': $t('cansat.mission.dashboard.controls.saveBtn'), 'placement': 'bottom'  }"
                    >
                      <div
                        class="btn btn-success btn-with-icon btn-micro rounded-icon"
                        style="margin: 0.1rem;"
                    ...
                    <vuestic-tooltip
                      v-if="isMissionActive && !finished"
                      :options="{ 'content': $t('cansat.mission.dashboard.controls.stopBtn'), 'placement': 'bottom'  }"
                    >
                      <div
                        class="btn btn-danger btn-with-icon btn-micro rounded-icon"
                        style="margin: 0.1rem;"
                      >
                    ...
                    <vuestic-tooltip
                      v-if="finished"
                      :options="{ 'content': $t('cansat.mission.dashboard.controls.exportBtn'), 'placement': 'bottom'  }"
                    >
                      <div
                        class="btn btn-info btn-with-icon btn-micro rounded-icon"
                        style="margin: 0.1rem;"
                      >
            ...
            <br>
            <small v-if="isMissionActive || finished">{{$t('cansat.mission.exportTitles.timeAxis')}}</small>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    getCanSatStatus() {
      ...
      if (
        this.$store.getters.axtec.project.cansat[0].connected &&
        this.$store.getters.axtec.project.cansat[0].testOk
      ) {
