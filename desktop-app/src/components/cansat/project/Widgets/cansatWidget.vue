      ...
      <div v-if="!isCanSatConnect">
        ...
        >
          <div class="input-group">
            ...
            <label class="control-label">{{$t('cansat.project.new.wizard.stepOne.name')}}</label>
            <i class="bar"></i>
            <small
              v-show="!isFormPathValid(canSatName)"
              class="help text-danger"
            >{{$t('cansat.project.new.wizard.stepOne.invalidName')}}</small>
          </div>
        </div>
      </div>
      <div v-else>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        { title: "cansat.link.table.name" },
        { title: "RSSI" },
        { title: "LQI" },
        { title: "Batt" },
        { title: "cansat.link.table.action" }
      ],
  ...
  methods: {
    setStatusesOnConnect() {
      ...
      this.$store.commit("setSignalCanSat", {
        index: 0,
    ...
    connect() {
      if (
        this.canSatName != "" &&
        (this.selectedCansat != "" || this.disableIDList)
      ) {
    ...
    resetActuators() {
      this.$store.commit(
        "setActuators",
        defaultActuators.actuators[defaultActuators.parachute]
      );
      this.$store.commit(
        "setActuators",
        defaultActuators.actuators[defaultActuators.balloon]
      );
    },
    resetSensors() {
